...
import type {
  DueTargetRaceFinalization,
  TargetRaceDeadlineFinalizationRepository,
} from '../../features/competitions/finalize-target-race-deadline.js';
...
export class PostgresTargetRaceDeadlineFinalizationRepository implements TargetRaceDeadlineFinalizationRepository {
  public constructor(
    private readonly database: Database,
    private readonly now: () => Date = () => new Date(),
  ) {}

  public async claimDueFinalization(): Promise<DueTargetRaceFinalization | undefined> {
    return this.database.transaction(async (transaction) => {
      ...
      if (competition.endsAt === null) return undefined;
      const [pendingClaim] = await transaction
        .select({ id: competitionTargetClaims.id })
        .from(competitionTargetClaims)
        .where(
          and(
            eq(competitionTargetClaims.guildId, competition.guildId),
            eq(competitionTargetClaims.competitionId, competition.id),
            eq(competitionTargetClaims.status, 'pending'),
            lte(competitionTargetClaims.receivedAt, competition.endsAt),
          ),
        )
        .limit(1);
      if (pendingClaim !== undefined) return undefined;
  ...
  > {
    return this.database.transaction(async (transaction) => {
      ...
      if (competition?.state !== 'finish_pending' || competition.winningTargetClaimId !== null)
        return { kind: 'finish_locked' };
      ...
      for (const finalValue of request.finalValues) {
        ...
        if (start === undefined) return { kind: 'finish_locked' };
...
function dueFinishCondition(now: Date) {
  return and(
    ...
    isNull(competitions.winningTargetClaimId),
    or(eq(competitions.type, 'skill_xp_target_race'), eq(competitions.type, 'boss_kc_target_race')),
